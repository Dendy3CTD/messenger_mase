-- +goose Up
-- Baseline: схема, которую до появления миграций создавала функция migrate() (сервер 0.3.0).
-- Все операторы идемпотентны, поэтому на уже существующей БД baseline только записывает версию 1.
...
-- +goose Down
-- Baseline не откатывается: откат удалил бы все таблицы и данные (`server migrate down` отказывает ниже версии 1).
